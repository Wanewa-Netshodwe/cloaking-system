...
const calculateMetrics = (data: AttendanceData[]) => {
  ...
  data.forEach((record) => {
    if (isOnTime(record.clock_in)) {
      ontime++;
      ontime_dataset.push(
        record.clock_in.getHours() + record.clock_in.getMinutes()
      );
    } else {
      late++;
      late_dataset.push(
...
const calculateMetricsDaily = (data: AttendanceData) => {
  console.log("calculateMetricsDaily - Processing single record:", data);
  ...
  if (isOnTime(data.clock_in)) {
    ontime++;
    ontime_dataset.push(data.clock_in.getHours() + data.clock_in.getMinutes());
  } else {
    late++;
    late_dataset.push(data.clock_in.getHours() + data.clock_in.getMinutes());
  }
  lunch_hours += 1;
  lunch_hours_dataset.push(1);
  working_hours = {
...
// Modified to find the next available key if today's key is not found
const getDataForDay = (
  attendance_data_map: Map<string, AttendanceData>,
  date: Date
) => {
