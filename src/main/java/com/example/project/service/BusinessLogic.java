...
public class BusinessLogic {
    ...
    public User correctPassword(User user ){
        Optional<User> u = userRepository.findByStudentNumberAndPassword(user.getFullName(), user.getPassword());
        if (u.isPresent()) {
    ...
    public ResponseEntity<String> clockIn(User user) {
        Optional<User> usr = userRepository.findByStudentNumberAndPassword(user.getFullName(), user.getPassword());

        if(usr.isPresent()){
            User existingUser = usr.get();
            Optional<Attendance> attendance = attendanceRepository.findByUserId(existingUser.getId());
            if (attendance.isPresent()) {
                return ResponseEntity.status(455).build();
            }else{
                Optional<User> u = userRepository.findById(existingUser.getId());
                if (u.isPresent()) {
                    Date now = new Date();
